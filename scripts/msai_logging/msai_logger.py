...
class Logger(metaclass=MiaoshouSingleton):
    ...
    def _do_init(self, log_folder: str, disable_console_output: bool = True) -> None:
        ...
        logging.basicConfig(level=os.environ.get("SD_LOGLEVEL", "ERROR"),
                            filemode='w',
                            format='%(asctime)s - %(filename)s [line:%(lineno)d] - %(levelname)s: %(message)s')

        # Setup info logging
        self._dataset[Logger.KEY_INFO] = logging.getLogger(Logger.KEY_INFO)
        msg_handler = logging.FileHandler(os.path.join(log_folder, "error.log"),
                                          "a",
                                          encoding="UTF-8")

        msg_handler.setLevel(level=os.environ.get("SD_LOGLEVEL", "ERROR"))
        ...
        error_handler = logging.FileHandler(
            ...
            mode="a",
            encoding='UTF-8')
        error_handler.setLevel(os.environ.get("SD_LOGLEVEL", "ERROR"))
    ...
    def __init__(self, log_folder: str = None, disable_console_output: bool = True) -> None:
